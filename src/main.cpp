...
int main(int argc, char* argv[])
{
    ...
    GLFWwindow* window;
    window = glfwCreateWindow(800, 600, "N2 Racing", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
    ...
    LoadTextureImage("../../data/truck.jpg");      // TextureImage1
    LoadTextureImage("../../data/tree.jpg");       // TextureImage2
    ...
    ObjModel truckmodel("../../data/truck.obj");
    ComputeNormals(&truckmodel);
    BuildTrianglesAndAddToVirtualScene(&truckmodel);
    ...
    ObjModel tiremodel("../../data/tire.obj");
    ComputeNormals(&tiremodel);
    BuildTrianglesAndAddToVirtualScene(&tiremodel);
    ...
    // Variáveis usadas para marcar o tempo
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // Cálculo do tempo decorrido desde o frame anterior
        t_now = glfwGetTime();
        deltaT = t_now - t_prev;
        t_prev = t_now;
        ...
        for(int i = 1; i < tireModels.size(); i=i+2) {
            model = tireModels[i];
            glUniformMatrix4fv(model_uniform, 1 , GL_FALSE , glm::value_ptr(model));
            glUniform1i(object_id_uniform, TIRE2);
            DrawVirtualObject("tire");
        }
...
void LoadTextureImage(const char* filename)
{
    ...
    glSamplerParameterf(sampler_id, GL_TEXTURE_MAX_ANISOTROPY, 16.0f);
...
void LoadShadersFromFiles()
{
    ...
    object_mlt              = glGetUniformLocation(program_id, "materials");
...
void KeyCallback(GLFWwindow* window, int key, int scancode, int action, int mod)
{
    ...
    if(key == GLFW_KEY_S)
    {
        pressedS = (!action == GLFW_RELEASE);
    }
    if(key == GLFW_KEY_A)
    {
        pressedA = (!action == GLFW_RELEASE);
    }
    if(key == GLFW_KEY_D)
    {
        pressedD = (!action == GLFW_RELEASE);
    }
